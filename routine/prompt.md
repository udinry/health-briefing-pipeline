...
Replace `<<BOT_TOKEN>>`, `<<CHAT_ID>>`, the timezone, and the device notes before use. If your environment supports environment variables, put the token there and reference `$TELEGRAM_BOT_TOKEN` instead of pasting it.
...
6. Anything unusual: outliers, a run of short nights, a sudden drop in activity, untracked nights.
Only state patterns the numbers actually support. Quote the supporting numbers. If a comparison is not possible because of thin history, say so briefly.
...
<b>Yesterday</b>: 4 lines: steps, active kcal, avg heart rate, sleep as Xh Ym with deep and REM in hours and their percentages. Each with an arrow vs the 7-day average (↑ up, ↓ down, → within 5%) and the average shown.
...
Never invent numbers. Round sensibly.
...
IF DATA IS MISSING
If yesterday has no row with a non-null steps or sleep value, do not summarize. Send exactly: "⚠️ Health sync appears broken: no data for <yesterday's date>. Check the Health Sync shortcut automation on the iPhone."
